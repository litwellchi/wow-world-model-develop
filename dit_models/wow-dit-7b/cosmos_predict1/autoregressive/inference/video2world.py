...
from cosmos_predict1.autoregressive.utils.inference import add_common_arguments, load_vision_input, validate_args, load_compress_videos
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default="1.0",
        help="Mask ratio for autoregressive video2world",
    )
    parser.add_argument(
        "--mask_type",
        type=str,
        default="uniform",
        choices=["uniform", "random", "continous", "similar"],
        help="Mask type for autoregressive video2world",
    )
    parser.add_argument(
        "--compression",
        action="store_true",
        help="Whether doint compress task",
    )
...
def main(args):
    ...
    if compress:
        video, _, _ = torchvision.io.read_video(args.input_image_or_video_path, pts_unit="sec")
        total_frames = video.shape[0]
        output_vids = []
        for i in range(0, total_frames, 33):
            if i + 33 > total_frames:
                break
            inp_vid = video[i:i+33]
            # Generate video
            input_videos, gt_videos = load_compress_videos(
                inp_vid,
                data_resolution=args.data_resolution,
                num_input_frames=args.num_input_frames,
                video_path=args.input_image_or_video_path,
            )
            prompt_entry = {"visual_input": args.input_image_or_video_path, "prompt": args.prompt}
            ...
            gt_vid  = gt_videos[input_filename]
            ...
            output_vids.append(out_vid)
        # concat output_vids
        out_vid = np.concatenate(output_vids, axis=0)
        ...
        imageio.mimsave(out_vid_path, out_vid, fps=5)

        log.info(f"Saved video to {out_vid_path}")
        return

    # Load input image(s) or video(s)
    input_videos, gt_videos = load_vision_input(
