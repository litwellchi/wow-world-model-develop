...
class ARBaseGenerationPipeline(BaseWorldGenerationPipeline):
    ...
    def __init__(
        ...
        # model_ckpt_path = "/Data/pd/Code/cosmos/ckpts/cosmos-ar-5b-ft/merged/model.pt"
        # model_ckpt_path = "/Data/pd/Code/cosmos/ckpts/cosmos-ar-5b-ft/step_19700_merged/model.pt"
        ...
        # tokenizer_ckpt_path = os.path.join(checkpoint_dir, "Cosmos-Tokenize1-DV8x16x16-720p/autoencoder.jit")
    ...
    def generate_partial_tokens_from_data_batch(
        ...
        data_tokens, token_boundaries, data_tokens_gt = self.model.tokenizer.tokenize(data_batch=data_batch)
        data_tokens = misc.to(data_tokens, "cuda").detach().clone()
        data_tokens_gt = misc.to(data_tokens_gt, "cuda").detach().clone()
        if parallel_state.get_context_parallel_world_size() > 1:
            data_tokens = get_batch_on_this_cp_rank(data_tokens)
            data_tokens_gt = get_batch_on_this_cp_rank(data_tokens_gt)
        batch_size = data_tokens.shape[0]

        for sample_num in range(batch_size):
            input_tokens = data_tokens[sample_num][0 : token_boundaries["video"][sample_num][1]]  # [B, L]
            input_tokens_gt = data_tokens_gt[sample_num][0 : token_boundaries["video"][sample_num][1]]  # [B, L]
            input_tokens = [
                input_tokens[0 : -num_tokens_to_generate - num_eov_tokens].tolist()
            ]# -1 is to exclude eov token
            input_tokens_gt = [input_tokens_gt.tolist()]
            log.debug(
    ...
    def generate_video_from_tokens(
        ...
        gt_tokens: Optional[list[torch.Tensor]] = None,
        mask_ratio: float = 0.0,
        mask_type: str = "uniform",
...
class ARVideo2WorldGenerationPipeline(ARBaseGenerationPipeline):
    ...
    def _run_model_with_offload(
        ...
        seed: int,
        sampling_config: SamplingConfig,
        gt_vid: Optional[torch.Tensor] = None,
        ...
        out_videos, indices_tensor, prompt_embedding = self._run_model(
            prompt_embedding, prompt_mask, inp_vid, num_input_frames, seed, sampling_config, gt_vid
        )
        if self.offload_network:
            self._offload_network()
        if self.offload_tokenizer:
    ...
    def _run_model(
        ...
        if gt_vid is not None:
            data_batch["gt_video"] = gt_vid
            data_batch["gt_video"] = data_batch["gt_video"].repeat(batch_size, 1, 1, 1, 1)
