...
class DiscreteMultimodalTokenizer:
    ...
    def _tokenize_video(self, videos: torch.Tensor, pixel_chunk_duration: Optional[int] = None):
        ...
        quantized_out, _ = self.video_tokenizer.encode(videos, pixel_chunk_duration=pixel_chunk_duration) # B C T H W
    ...
    def tokenize(self, data_batch: dict):
        ...
        if (
            ...
            tokens_gt = self._tokenize_text(data_batch["caption"], max_text_seq_len)
            for i in range(batch_size):
        ...
        if "class" in self.training_type and self.tokenizer_config.class_tokenizer is not None:
            ...
            if len(tokens) == 0:
                ...
                for i in range(batch_size):
                    ...
                    tokens_gt[i] = tokens_gt[i] + tokens_class[i]
        ...
        if self.video_tokenizer is not None and self.tokenizer_config.video_tokenizer.tokenize_here:
            ...
            tokens_gt_video = self._tokenize_video(data_batch["gt_video"], pixel_chunk_duration=pixel_chunk_duration)
            if len(tokens) == 0:
                tokens = tokens_video
                tokens_gt = tokens_gt_video
                for i in range(batch_size):
                ...
                for i in range(batch_size):
                    ...
                    tokens_gt[i] = tokens_gt[i] + tokens_gt_video[i]
        ...
        for i in range(len(tokens)):
            if len(tokens[i]) < pad_to_len:
                ...
                tokens_gt[i] = tokens_gt[i] + [self.pad_id] * (pad_to_len - len(tokens_gt[i]))
            else:
                tokens[i] = tokens[i][0:pad_to_len]
                tokens_gt[i] = tokens_gt[i][0:pad_to_len]

        # Convert it to long tensor
        tokens = torch.LongTensor(tokens)
        tokens_gt = torch.LongTensor(tokens_gt)
        return tokens, token_boundaries, tokens_gt
