...
_SUPPORTED_CONTEXT_LEN = [1, 9, 17, 25]  # Input frames
NUM_TOTAL_FRAMES = 33
...
def load_compress_videos(video, data_resolution: List[int], num_input_frames: int, video_path: str) -> dict:
    all_videos = dict()
    gt_videos = dict()
    ...
    gt_video = resize_input(gt_video, data_resolution)
    
    fname = os.path.basename(video_path)
    all_videos[fname] = video.transpose(0, 1).unsqueeze(0) # B C T H W - > B T C H W
    gt_videos[fname] = gt_video.transpose(0, 1).unsqueeze(0)
    return all_videos, gt_videos
...
def load_videos_from_list(flist: List[str], data_resolution: List[int], num_input_frames: int) -> dict:
    ...
    for video_path in flist:
        ...
        if ext in _VIDEO_EXTENSIONS:
            ...
            if video.shape[0] > NUM_TOTAL_FRAMES:
                # video = video[0:num_frames, :, :, :]
                total_frames = video.shape[0]

            gt_video = video.clone()
            
            #OLD # video = video[-num_input_frames:, :, :, :]
            video = video[:num_input_frames, :, :, :]
...
def load_vision_input(
    ...
    if batch_input_path:
        ...
        if input_type == "image" or input_type == "text_and_image":
            ...
            input_videos, gt_videos = read_input_videos(
                batch_input_path,
        ...
        if input_type == "image" or input_type == "text_and_image":
            ...
            input_videos, gt_videos = read_input_video(
                input_image_or_video_path,
                data_resolution=data_resolution,
                num_input_frames=num_input_frames,
            )
