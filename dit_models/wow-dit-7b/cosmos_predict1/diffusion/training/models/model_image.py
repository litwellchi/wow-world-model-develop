...
# from cosmos_predict1.vjepa2.vision_transformer import VIT_EMBED_DIMS, vit_giant
...
class DiffusionModel(Model):
    def __init__(self, config):
        ...
        # --------------------------------------- ROBOREPA -------------------------------- +
        # self._vit_giant = self.load_vjepa()
        # self.projectors = nn.ModuleList([
        #     build_mlp(4096, 2048, z_dim) for z_dim in [1408]
        #     ])
        # self.projectors.cuda()
        # self.projectors = self.projectors.to(torch.bfloat16)
        # --------------------------------------- ROBOREPA -------------------------------- -
    ...
    # ------------------------------------------------------------ ROBOREPA ------------------------------------ +
    # def load_vjepa(self):
    #     _vit_giant = vit_giant(img_size=384,num_frames=8, tubelet_size=2, use_rope=True)
    #     checkpoint = "/mnt/pd/Code/vjepa2/ckpts/vitg-384.pt"
    #     ckpt = torch.load(checkpoint)
    #     new_state_dict = OrderedDict()
    #     for k, v in ckpt["encoder"].items():
    #         name = k[16:] if k.startswith('module.backbone.') else k
    #         new_state_dict[name] = v

    #     _vit_giant.load_state_dict(new_state_dict)
    #     _vit_giant.cuda()
    #     _vit_giant = _vit_giant.to(torch.bfloat16)
    #     _vit_giant.eval()
    #     return _vit_giant
        
    # def get_vjepa_feature(self, x0_from_data_batch):
    #     with torch.no_grad():
    #         raw_video = x0_from_data_batch[0].permute(1, 0, 2, 3)
    ...
    #         #     [110, 112, 114, 116, 118, 120, 120, 120]]
    #         frame_indices = [[0, 4, 8, 12, 15, 19, 23, 27],
    #             [31, 35, 39, 43, 46, 50, 54, 58],
    #             [62, 66, 70, 73, 77, 81, 85, 89],
    #             [93, 97, 101, 105, 108, 112, 116, 120]]
    #         x_list = []
    #         for indices in frame_indices:
    #             cur_video = raw_video[indices].unsqueeze(0)
    #             cur_video = cur_video.permute(0,2,1,3,4)
    #             x = self._vit_giant(cur_video)
    #             T, H, W = 4, 24, 24
    #             x_reshaped = x.reshape(1, T, H, W, 1408)
    #             # 上采样到 (B, T, 24, 24, D)
    #             # x_permuted = x_reshaped.permute(0, 1, 4, 2, 3)  # (B, T, D, H, W)
    #             # x_upsampled = F.interpolate(
    #             #     x_permuted, size=(24, 24), mode='bilinear', align_corners=True
    #             # )
    #             # output = x_upsampled.permute(0, 1, 3, 4, 2)  # (B, T, 24, 24, D)
    ...
    def compute_loss_with_epsilon_and_sigma(
        ...
        # ------------------------------------------- ROBOREPA ------------------------------------- +
        # feature = model_pred.feature
        # # print(feature.shape)
        # vjepa_feature = self.get_vjepa_feature(x0_from_data_batch)
        # # print(vjepa_feature.shape)
        # feature_proj = [projector(feature.reshape(1, -1, 4096)).reshape(1, 16, 24, 24, 1408) for projector in self.projectors]
        # feature_proj = feature_proj[0]
        # # get understanding loss
        # cosine_similarities = -F.cosine_similarity(feature_proj, vjepa_feature, dim=-1)
        # loss_repa = torch.mean(cosine_similarities) * 0.75
        # # add to edmloss
        ...
        output_batch = {
            ...
            # ------------------------------------------- ROBOREPA ------------------------------------- +
            # "edm_loss": edm_loss.mean() + loss_repa,
            "edm_loss": edm_loss.mean(),
            # ------------------------------------------- ROBOREPA ------------------------------------- -
        }
        # ------------------------------------------- ROBOREPA ------------------------------------- +
        self.wandb_log(kendall_loss, pred_mse, edm_loss)
        #  self.wandb_log(kendall_loss, pred_mse, edm_loss, loss_repa)
        # ------------------------------------------- ROBOREPA ------------------------------------- -
        # print(f"kendall_loss: {kendall_loss} pred_mse:{pred_mse} edm_loss:{edm_loss}")
        return output_batch, kendall_loss, pred_mse, edm_loss

    # ------------------------------------------- ROBOREPA ------------------------------------- +
    # @rank0_only
    # def wandb_log(self, kendall_loss, pred_mse, edm_loss, loss_repa):
    #     wandb.log({"kendall_loss": kendall_loss.mean(), "mse_loss": pred_mse.mean(), "edm_loss": edm_loss.mean(), "loss_repa": loss_repa.mean()})
    
    @rank0_only
    def wandb_log(self, kendall_loss, pred_mse, edm_loss):
        wandb.log({"kendall_loss": kendall_loss.mean(), "mse_loss": pred_mse.mean(), "edm_loss": edm_loss.mean()})
    # ------------------------------------------- ROBOREPA ------------------------------------- -    
    def training_step(
    ...
    def denoise(self, xt: torch.Tensor, sigma: torch.Tensor, condition: CosmosCondition) -> DenoisePrediction:
        ...
        #     net_output, features = net_output
        ...
        # ---------------------------- RoboREPA ------------------------- +
        # REPA: # return DenoisePrediction(x0_pred, eps_pred, logvar, feature)
...
def diffusion_fsdp_class_decorator(base_class: Type[T]) -> Type[T]:
    ...
    class FSDPClass(base_class):
        ...
        def state_dict_model(self) -> Dict:
            ...
            with FSDP.summon_full_params(self.model, writeback=False, offload_to_cpu=True, rank0_only=True):
                pass
            with FSDP.state_dict_type(
                self.model, StateDictType.FULL_STATE_DICT, FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
            ):
                model_state = self.model.state_dict()
                # print("STATE DEVICES", next(iter(model_state.values())).device)
            if self.config.ema.enabled:
                # with FSDP.summon_full_params(self.model_ema):
                with FSDP.summon_full_params(self.model_ema, writeback=False, offload_to_cpu=True, rank0_only=True):
                    pass
                with FSDP.state_dict_type(
                    self.model_ema,
                    ...
                    # print("STATE DEVICES", next(iter(ema_model_state.values())).device)
            else:
